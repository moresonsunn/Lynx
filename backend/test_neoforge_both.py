# Test NeoForge mod with side=BOTH
...
with tempfile.TemporaryDirectory() as tmpdir:
    jar_path = Path(tmpdir) / 'neoforge-mod.jar'
    with zipfile.ZipFile(jar_path, 'w') as zf:
        toml_content = 'modLoader="javafml"\nloaderVersion="[36,)"\nlicense="MIT"\n[[mods]]\nmodId="neoforgemod"\nversion="1.0.0"\ndisplayName="NeoForge Mod"\nside="BOTH"\n'
        zf.writestr('META-INF/neoforge.mods.toml', toml_content)
        zf.writestr('com/example/NeoForgeMod.class', bytes.fromhex('CAFEBABE00000034000100000000'))
    
    mod = extract_jar(jar_path)
    print(f'Extracted: {mod.canonical_id}, side={mod.declared_side.value}, loaders={mod.loaders}')
    
    ctx = AnalysisContext(loader='neoforge', mc_version='1.20.1')
    ...
    print('NeoForge BOTH test passed!')
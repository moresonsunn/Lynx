# Test Forge mod with explicit side=SERVER
...
with tempfile.TemporaryDirectory() as tmpdir:
    jar_path = Path(tmpdir) / 'forge-server-mod.jar'
    with zipfile.ZipFile(jar_path, 'w') as zf:
        toml_content = 'modLoader="javafml"\nloaderVersion="[36,)"\nlicense="MIT"\n[[mods]]\nmodId="forgeservermod"\nversion="1.0.0"\ndisplayName="Forge Server Mod"\nside="SERVER"\n'
        zf.writestr('META-INF/mods.toml', toml_content)
        zf.writestr('com/example/ForgeServerMod.class', bytes.fromhex('CAFEBABE00000034000100000000'))
    ...
    print('Forge SERVER side test passed!')